...
class CustomToastHandler : public IWinToastHandler {
 public:
  ...
  void toastReplied(const std::wstring& text) const {
    std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> converter;
    flutter::EncodableMap args = flutter::EncodableMap();
    args[flutter::EncodableValue("notificationId")] =
        flutter::EncodableValue(identifier);
    args[flutter::EncodableValue("text")] =
        flutter::EncodableValue(converter.to_bytes(text));
    channel->InvokeMethod("onLocalNotificationReply",
                          std::make_unique<flutter::EncodableValue>(args));
  }
...
void LocalNotifierPlugin::Notify(
  ...
  auto reply = args.find(flutter::EncodableValue("replyPlaceholder"));
  if (reply != args.end() && std::holds_alternative<std::string>(reply->second)) {
    std::string placeholder = std::get<std::string>(reply->second);
    if (!placeholder.empty()) {
      toast.setReplyPlaceholder(converter.from_bytes(placeholder));
    }
  }
